...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create(table: 'orden', callback: function (Blueprint $table) {
            ...
            $table->enum('tipo_transporte', ['buque', 'train', 'truck']);
            $table->foreignId('id_administrativo')-> constrained('users') -> onDelete('cascade');
            $table->foreignId('id_buque')->nullable()->constrained('buque');
            $table->foreignId('id_train')->nullable()->constrained('trains');
            $table->foreignId('id_truck')->nullable()->constrained('trucks');
